# 📖 축의금 마스터 (ChuguiMaster v2.0.0) 최종 최신화 사용 설명서

> 🔒 **개인정보 보호(Anonymization) 완벽 보장**: 본 설명서에 포함된 모든 이미지 및 수치(`1,250,000원`, `5건` 등)는 이번에 새로 도입된 **Qt `widget.grab()` 동적 익명 UI 캡처 기술**을 통해 실제 하객 개인정보 노출 없이 100% 가상 데이터(`홍길동`, `김철수`, `이영희`, `박민수`, `최동욱`)로 자동 생성된 화면입니다.

---

## 1. 📊 한눈에 보는 컴팩트 스마트 대시보드

![동적 캡처 메인 대시보드](docs/images/dashboard.png)
...
![식대 단가 설정 캡처](docs/images/meal_cost.png)
...
![동적 캡처 하객 명단 테이블](docs/images/table.png)
...
## 5. 📸 [신규 도입] `widget.grab()` 익명 UI 스크린샷 자동 캡처 서비스
