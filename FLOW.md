...
> v2.1.0 기준. 이 문서는 **코드가 실제로 하는 일**을 기술한다.
...
그래서 전체 테스트 444개 중 GUI가 필요한 것은 표 모델·UX 테스트뿐이고,
...
되돌려야 할 일은 두 종류다. 성격이 다르므로 장치도 다르다.

| 사고 | 장치 | 최대 손실 |
|---|---|---|
| 전원 차단 · 강제 종료 | 원자적 자동 저장 + `.bak` | 600ms |
| 셀 하나 잘못 고침 | `Ctrl+Z` (QUndoStack) | 없음 |
| 비우기 · 덮어쓰기 오클릭 | 토스트 `되돌리기` (9초) | 없음 |
| 한참 전 상태로 돌아가고 싶음 | 시점 스냅샷 (최대 20개) | 최대 5분 |

```mermaid
flowchart TD
    A["파괴 연산 요청<br/>비우기 · 덮어쓰기 · 병합 · 샘플"] --> B["_begin_destructive()"]
    B --> C["현재 상태를 스냅샷 파일로 기록"]
    B --> D["현재 상태를 메모리에 보관<br/>_undo_payload"]
    C & D --> E["연산 수행"]
    E --> F["토스트 + [되돌리기] 버튼 9초"]
    F -- "클릭" --> G["_restore_payload()<br/>입력창 + 표 + 세션 복원"]
    F -- "시간 초과" --> H["메모리 보관분 폐기<br/>스냅샷은 계속 남음"]

    I["5분 주기 타이머"] --> C
    J["[이전 시점 복구] 버튼"] --> K["스냅샷 목록<br/>시각 · 사유 · 건수 · 총액"]
    K --> L["복구 직전에도 스냅샷 기록"]
    L --> G
```

**`전체 비우기` 는 세션 파일과 `.bak` 을 지우지만 스냅샷은 남긴다.**
v2.0에서 이 조합은 되돌릴 방법이 전혀 없었다.

### 비정상 종료 감지

```mermaid
flowchart LR
    A["기동"] --> B{"running.lock<br/>존재?"}
    B -- 예 --> C["지난 실행 = 비정상 종료"]
    B -- 아니오 --> D["지난 실행 = 정상"]
    C & D --> E["표식 새로 기록<br/>pid + 시작 시각"]
    E --> F["세션 복구"]
    F --> G{"비정상이었나?"}
    G -- 예 --> H["안내 + [이전 시점 보기]"]
    I["정상 종료"] --> J["표식 삭제"]
...
앱은 자기가 지난번에 어떻게 끝났는지 모른다. 표식이 없으면
"지난번에 갑자기 꺼졌습니다. 작업은 복구했습니다"라고 말해 줄 수 없고,
사용자는 자기 작업이 살아 있는지 모른 채 불안해한다.

### 되돌리기 스택 위생

- 목록이 통째로 바뀌면(`set_guests`) 스택을 비운다 — 행 번호가 가리키던 대상이 사라진다
- 거부되는 편집(빈 이름)은 명령으로 쌓지 않는다
- 값이 그대로인 편집도 쌓지 않는다
- 참석 → 불참 변경은 식권을 0으로 만든다. 되돌리면 식권도 함께 돌아온다
