...
class SmartParser:
    ...
    RELATION_KEYWORDS = {
        '친척/가족': ['이모', '할머니', '삼촌', '고모', '숙부', '친척', '외삼촌', '가족', '친지', '처남', '처형', '당숙'],
        '직장/기관': ['보건소', '보건지소', '보건지구', '지소', '병원', '회사', '팀장', '부장', '과장', '대리', '사원', '대표', '동료', '직장', '소속', '주임', '계장'],
        '종교/모임': ['교회', '성당', '사찰', '모임', '선교회', '구역', '집사', '권사', '장로', '목사', '신부', '스님'],
    ...
    def parse_single_line(cls, line: str, line_num: int) -> Dict[str, Any]:
        ...
        raw_name = f"하객{line_num}"
        for token in tokens:
            cleaned = re.sub(r'[\d만,원식권불참송금현금계좌이체]', '', token).strip()
            if cleaned and len(cleaned) >= 2 and not any(kw in cleaned for kw in ['보건', '교회', '이모', '친척', '할머니', '친구', '동기', '지인']):
                raw_name = token
                break
